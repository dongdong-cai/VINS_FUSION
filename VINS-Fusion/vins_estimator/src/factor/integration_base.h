...
class IntegrationBase
{
  public:
	...
	/**
     * @brief 记录用于IMU积分的数据；并调用propagate()函数进行预积分计算，这里会计算delta_PVQ及其雅可比矩阵
    */
	...
	/**
	 * @brief 依据新的IMU的加速度和角速度偏置值，重新IMU预积分
	 * @param _linearized_ba 新的加速度零偏
	 * @param _linearized_bg 新的陀螺仪零偏
	 */
	...
	/**
     * @brief 中值法计算IMU预积分，这里的预积分结果是当前时刻到上一图像帧的相对P、V、Q，因为初始化时将上一图像帧的PVQ都设置为了单位矩阵
    ...
    */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
		...
		// 用当前帧imu数据更新P V Q（感觉这段IMU积分的代码重复出现和运行了好多次）
	 ...
	 * @param _dt 当前帧IMu的时间间隔
	 * @param _acc_1 当前帧IMU的加速度
	 * @param _gyr_1 当前帧IMU的角速度
	 */
    ...
    double sum_dt;
	// delta_p、delta_q、delta_v是从上一图像帧到上一时刻的IMU积分值
